...
#####
setwd('~/Desktop/gpr_testing/nyc_sales/')

data = fread('./out_dummy1.csv')
data$V1 = NULL
ds = as.data.frame(data[,c(1:6,8:61,7)])
...
ds_test = ds[test_idx,]
ds_test_y = ds_test$`SALE PRICE`

ds2 = as.data.frame(ds)
# normalize to mean 0 and unit variance.
mean_shift = mean(log2(ds_train$`SALE PRICE`))
ds2$`SALE PRICE` <- log2(ds2$`SALE PRICE`) - mean_shift
...
ds2_train_x <- model.matrix(`SALE PRICE`~.-1, ds2_train)
ds2_train_y <- as.matrix(ds2_train$`SALE PRICE`)
ds2_test_x <- model.matrix(`SALE PRICE`~.-1, ds2_test)
ds2_test_y <- as.matrix(ds2_test$`SALE PRICE`)
